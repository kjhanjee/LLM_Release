...
class Trainer:
    """Module for creating trainer class to train the Pytorch LLM model
    """
    def __init__(self, model, data: TrainingDataGen, tokenizer: Tokenizer, optimizer:torch.optim.Optimizer = None, max_sequence_length: int = 4096, checkpoint_path: str = None, batch_size: int = 2):
    ...
    def sentence_iterator(self,sentences:list,path:str):
                ...
                while i < len(ids) - 1:
                    if i + self.max_sequence_length < len(ids) - 1:
                        ...
                        for _ in range(0,self.max_sequence_length-len(ids2) + 1):
                            ids2.append(24)
                        sequences.append(ids2)
                    i = i + int(self.max_sequence_length/4)
                    if len(sequences) == self.batch_size:
                        sequences2 = sequences
                        if self.iter_index >= self.step:
                            yield sequences2
                        sequences = []
                        self.iter_index += 1
            if sentence_no == len(sentences) - 1:
                self.data_generator.completed_files.append(path)
    ...
    def train(self, epochs:int, batch_size:int):
        ...
        with tqdm(total=100) as pbar:
            for epoch in tqdm(range(epochs),total = epochs):
                ...
                step = self.step
                for path, sentences_from_file in self.data_generator.gen_batch():
                    self.last_sentence_index = 0
                    for sequences2 in self.sentence_iterator(sentences_from_file,path):
                        self.model.train()
                        ...
                        if step % accum_iter == 0:
                            ...
                            self.data_generator.save_state(config['checkpoint_path'])
                            torch.save(self.model,config['checkpoint_path']+"model.bin")
                            print("All Saved!")
